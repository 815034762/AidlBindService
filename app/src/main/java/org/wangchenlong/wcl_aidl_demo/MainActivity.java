...
public class MainActivity extends AppCompatActivity {
    ...
    private IOnNewBookArrivedListener mOnNewBookArrivedListener = new IOnNewBookArrivedListener.Stub() {
        ...
        public void onNewBookArrived(Book newBook) {
            ...
            String temp = new String("张天洋");
            try {
                byte[] result = mRemoteBookManager.foo(temp.getBytes());
                boolean boolResult = (result[0] == 0x00)?false:true;
                byte[] bytes = new byte[result.length-1];
                for(int i=0;i<(result.length-1);i++){
                    bytes[i] = result[i+1];
                }
                String strResult = new String(bytes);
                Log.e(TAG, strResult + " result " + boolResult);
